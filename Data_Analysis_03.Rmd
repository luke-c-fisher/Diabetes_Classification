...
trainMatrx = model.matrix(Diabetes ~ .^2 + BMI^2 + GenHlth^3 + Age + HighBP + HighChol
                          -1, data = diabetesTrn)
testMatrx = model.matrix(Diabetes ~ .^2 + BMI^2 + GenHlth^3 + Age + HighBP + HighChol
                         -1, data = diabetesTst)
...
As we can see from the comparison, the boost model delivers more accurate results as a classifier compared to the standard model from before. 
...
tstBoostErr = calcErr(actual = diabetesTst$Diabetes, predicted = predLabels)
